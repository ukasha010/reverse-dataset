...
void std::basic_ostream<char,std::char_traits<char>>::seekp(long param_1,_Ios_Seekdir param_2)

{
  (*(code *)PTR_seekp_00103f88)();
  return;
}
...

{
  ...
  std::basic_ofstream<char,std::char_traits<char>>::basic_ofstream((char *)local_228,0x102004);
                    /* try { // try from 001012d6 to 00101363 has its CatchHandler @ 0010138b */
  cVar1 = std::basic_ios<char,std::char_traits<char>>::operator!(abStack_130);
  if (cVar1 == '\0') {
    std::basic_ostream<char,std::char_traits<char>>::seekp((long)local_228,0);
    pbVar2 = std::operator<<(local_228,"Virat Kohli");
    std::basic_ostream<char,std::char_traits<char>>::operator<<
...
{
                    /* WARNING: Bad instruction - Truncating control flow here */
                    /* _ZNSo5seekpElSt12_Ios_Seekdir@GLIBCXX_3.4 */
  halt_baddata();
}
